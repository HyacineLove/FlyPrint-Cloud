...
| 人类部署入口 | `README.md`；部署手册见 `../deliverables/05-部署与验证.md`（人类向，不参与开发参考） |
...
| `../deliverables/05-部署与验证.md` | 安装、配置、公网 :80 / 局域网、检查单与排障（人类向，不参与开发参考） | 环境变量与网络；第三方接入填值细节 → 第三方指南 §8；产品边界 → README |
| `../deliverables/06-第三方接入指南.md` | 已实现 HMAC 第三方接入的对接契约（对外交付） | Demo `code`=`livacloud-demo`；部署变量 / 三类地址 → 部署与验证、README；Site Portal 身份链路 → `docs/agent/site-portal-identity-protocol.md` |
...
- 改 Cloud-Edge 协议：同步 `message.go`、序列化测试、Cloud provider test（`docs/TESTING.md`）、Edge consumer test（`../../fly-print-edge/docs/TESTING.md`）。协议以 Go 源码为准，Swagger 不完整。
