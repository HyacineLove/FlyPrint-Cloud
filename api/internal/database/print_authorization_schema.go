...
// initPrintAuthorizationSchema 兼容直接调用 InitTables 的旧实例和测试环境。
// 正常服务启动仍由编号迁移记录正式的前向变更。
func (db *DB) initPrintAuthorizationSchema() error {
	statements := []string{
		`ALTER TABLE users
		...
		`ALTER TABLE print_jobs ADD COLUMN IF NOT EXISTS edge_node_id VARCHAR(100) REFERENCES edge_nodes(id)`,
		`ALTER TABLE print_jobs ADD COLUMN IF NOT EXISTS site_portal_code VARCHAR(64) REFERENCES site_portals(code)`,
		`ALTER TABLE print_jobs ADD COLUMN IF NOT EXISTS terminal_session_id VARCHAR(128)`,
		`ALTER TABLE print_jobs ADD COLUMN IF NOT EXISTS confirmation_id VARCHAR(128)`,
		`ALTER TABLE print_jobs ADD COLUMN IF NOT EXISTS authorization_request_hash CHAR(64)`,
		`ALTER TABLE print_jobs ADD COLUMN IF NOT EXISTS local_file_id VARCHAR(128)`,
		`ALTER TABLE print_jobs ADD COLUMN IF NOT EXISTS quota_reserved INTEGER NOT NULL DEFAULT 0`,
		`ALTER TABLE print_jobs ADD COLUMN IF NOT EXISTS quota_consumed INTEGER`,
		`ALTER TABLE print_jobs ADD COLUMN IF NOT EXISTS impressions_completed INTEGER`,
		`ALTER TABLE print_jobs ADD COLUMN IF NOT EXISTS sheets_completed INTEGER`,
	...
	for _, statement := range statements {
		if _, err := db.Exec(statement); err != nil {
			return fmt.Errorf("initialize print authorization schema: %w", err)
		}
	}
	return nil
}
